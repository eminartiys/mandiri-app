 ...
 */
class DiscoverMovieByUsecase @Inject constructor(
    private val repository: MovieRepository,
    dispatcher: CoroutineDispatcher
) : BaseUseCase<Pair<String, Int>, List<Movie>>(dispatcher) {

    override suspend fun execute(param: Pair<String, Int>?): Flow<ViewResource<List<Movie>>> {
        return repository.discoverMovieBy(param?.first.orEmpty(), param?.second).map {
            when (it) {
                is DataResource.Success -> {
                    if (it.data == null) {
                        ViewResource.Empty(listOf())
                    } else if (it.data.movies.isNullOrEmpty()) {
                        ViewResource.Empty(listOf())
                    } else {
                        ViewResource.Success(it.data.movies!!)
                    }
                }
